...
"""
Perform leave one out voxel selection with FCMA
"""
...
if __name__ == '__main__':
    ...
    # Take out the indices corresponding to all participants but this one
    subsampled_idx = list(set(range(len(labels))) - set(range(start_idx, end_idx)))
    
    # And use those indices to subsample labels and raw data
    ...
    results = vs.run(clf) # Results is a list of tuples, where idx0 is the voxel number and idx1 is its accuracy
    ...
    if MPI.COMM_WORLD.Get_rank()==0:
        ...
        seq_volume = np.zeros(mask.shape, dtype=np.int32)
        seq = np.zeros(len(results), dtype=np.int32)
        ...
        with open(file_str + 'result_list.txt', 'w') as fp:
            for idx, res_tuple in enumerate(results): 
                fp.write(str(res_tuple[0]) + ' ' + str(res_tuple[1]) + '\n')
                
                # Store the score for each voxel
                score[res_tuple[0]] = res_tuple[1]
                seq[res_tuple[0]] = idx
