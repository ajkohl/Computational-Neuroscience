...
data_path = '/gpfs/gibbs/project/cmhn/data/'
...
# NB: These are specific for the CMHN-S23 setup on Grace where students have a symlink in their homedir to their scratch!
scratch_directory = os.path.expanduser("~/palmer_scratch/") # this deletes everything after 60 days
results_path = os.path.join(scratch_directory, 'brainiak_results') # change these paths only if you want them to go elsewhere!
...
def load_vdc_stim_labels(sub):
    ...
    for run in range(1, vdc_n_runs + 1):
        in_file = os.path.join(vdc_data_dir, sub ,'ses-day2','design_matrix',f'{sub}_localizer_{run:02d}.mat')
...
def load_vdc_mask(ROI_name, sub):
    ...
    # load the mask
    maskfile = f"{maskdir}/{sub}_ventral_{ROI_name}_locColl_to_epi1.nii.gz"
    mask = nib.load(maskfile)
    print(f"Loaded {ROI_name} mask")
    return mask
...
def load_vdc_epi_data(sub, run):
    ...
    epi_in = os.path.join(vdc_data_dir, sub,
              "preprocessed", "loc", f"{sub}_filtered2_d1_firstExampleFunc_r{run}.nii")
    epi_data = nib.load(epi_in)
    print(f"Loading data from {epi_in}")
    return epi_data
...
# Make a function to load and mask VDC data
def load_vdc_masked_data(directory, subject_name, mask_list):
    ...
    for mask_idx in range(len(mask_list)):
        ...
        mask = load_vdc_mask(mask_list[mask_idx], subject_name)
        
        # hold all runs' data for this mask
        this_mask_data = []
        ...
        for run in range(1, vdc_n_runs + 1):
            ...
            # mask the data 
            epi_masked_data = mask_data(epi_data, mask).T
            this_mask_data.append(epi_masked_data)
        masked_data_all.append(np.hstack(this_mask_data, axis=0))
...
def load_data(directory, subject_name, mask_name=None, num_runs=3, zscore_data=False):
    
    # Cycle through the masks
    print ("Processing Start ...")
    
    # If there is a mask supplied, then load it now, otherwise wait until we have an EPI
    # and then define a mask
    if mask_name:
...
def load_labels(directory, subject_name):
    ...
    for run in range(1,4):
        in_file= os.path.join(directory, subject_name, 'ses-day2','design_matrix' ,f"{subject_name}_localizer_{run:02d}.mat")
